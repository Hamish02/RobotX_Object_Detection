...
void print_object_depth(const sl::Objects &objects, const sl::Mat &point_cloud) {
    for (const auto &object : objects.object_list) {
        std::cout << "Object ID: " << object.id << ", Label: " << object.label << ", Confidence: " << object.confidence << std::endl;

        // Get the 3D position of the object
        sl::float3 position;
        point_cloud.getValue(object.position.x, object.position.y, &position);
...
int main(int argc, char** argv) {
    ...
    while (viewer.isAvailable()) {
        if (zed.grab() == sl::ERROR_CODE::SUCCESS) {
            ...
            print_object_depth(objects, point_cloud);
